...
> = {
  [NotificationType.Outbid]: { icon: "⚡", color: "text-[#f0a500]", bg: "bg-[#f0a500]/10" },
  [NotificationType.Win]: { icon: "🏆", color: "text-green-400", bg: "bg-green-400/10" },
  [NotificationType.Payment]: { icon: "💳", color: "text-purple-400", bg: "bg-purple-400/10" },
};

const DEFAULT_TYPE_CONFIG = TYPE_CONFIG[NotificationType.Outbid];
...
export const NotificationItem = ({ notification, onMarkAsRead, index }: NotificationItemProps) => {
  const config =
    TYPE_CONFIG[notification.notificationType as NotificationType] ?? DEFAULT_TYPE_CONFIG;
  ...
  const actionLabel =
    notification.notificationType === NotificationType.Outbid
      ? "Place new bid"
  ...
  return (
    <div
      ...
      className={`group relative p-6 transition-all duration-500 flex gap-5 cursor-pointer overflow-hidden animate-bvCatFadeUp ${
        !notification.isRead
    ...
    >
      ...
      <div className="relative shrink-0">
        <div
          className={`w-12 h-12 rounded-2xl ${config.bg} flex items-center justify-center text-xl relative z-10 border border-white/[0.05] shadow-inner group-hover:animate-bvFloat`}
        >
          <span className={`${config.color} filter drop-shadow-[0_0_8px_currentColor]`}>
            {config.icon}
          </span>
        </div>
        {!notification.isRead && (
        ...
        <div className="flex justify-between items-start gap-4 mb-1.5">
          <div className="flex flex-col gap-0.5">
            ...
            <span className="font-mono text-[9px] text-brand-primary/50 uppercase tracking-[0.15em]">
              {String(notification.notificationType).replace("-", " ")}
            </span>
          </div>
